...
main() {
  ...
  *)
  ...
  # Status left
  if $show_powerline; then
    tmux set-option -g status-left "#[bg=${green},fg=${gray}]#{?client_prefix,#[bg=${yellow}],} ${left_icon} #[fg=${green},bg=${gray}]#{?client_prefix,#[fg=${yellow}],}${left_sep}"
    powerbg=${gray}
  else
  ...
  for plugin in "${plugins[@]}"; do
    ...
    if $show_powerline; then
      if $show_empty_plugins; then
        tmux set-option -ga status-right "#[fg=${!colors[0]},bg=${powerbg},bold,nounderscore,noitalics]${right_sep}#[fg=${!colors[1]},bg=${!colors[0]}]$script "
      else
        tmux set-option -ga status-right "# {?#{==:$script,}h[fg=${!colors[0]},bold,nounderscore,noitalics]${right_sep}# [fg=${gray},bg=${gray}]$script}"
      fi
      powerbg=${!colors[0]}
    else
      if $show_empty_plugins; then
  ...
  # Window option
  if $show_powerline; then
    tmux set-window-option -g window-status-current-format "#[fg=${gray},bg=${dark_purple}]${left_sep}#[fg=${white},bg=${dark_purple} , bold] #W${current_flags} #[fg=${dark_purple},bg=${gray}]${left_sep}"
  fi

  tmux set-window-option -g window-status-format " #W${flags} #[fg=${gray},bg=${gray}]${left_sep}"
